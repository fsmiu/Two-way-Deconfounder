...
parser = argparse.ArgumentParser(description='tumor case eval')
...
u, t = args.d_number, args.d_t-1
h_dim = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['h_dim']

z_dim = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['z_dim']

alpha = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['alpha']

embedding_size = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['embedding_size']

batch_size = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['batch_size']

learning_rate = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['learning_rate']

w_decay = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
        args.c_degree) + '_tune'+method]['w_decay']

lamda = load_yaml('tables/tumor/op_hyper_params.yml', key='tumor')[
    str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
    ...
    for j in range(t):
        ...
        if args.random:
            ...
            for i in range(u):
                if s[i, 0] > 88:
                    ap[i] = 0.2
                elif s[i, 0] <= 88 and s[i, 0] > 44:
                    ap[i] = 0.1
                elif s[i, 0] <= 44 and s[i, 0] > 5:
                    ap[i] = 0.05
                else:
                    ap[i] = 0.01
        a1 = torch.distributions.bernoulli.Bernoulli(ap)
        a1 = a1.sample()
        a2 = torch.distributions.bernoulli.Bernoulli(ap)
        a2 = a2.sample()
        a1 = a1.view(-1, 1)
        a2 = a2.view(-1, 1)
        a = torch.cat((a1, a2), 1)
