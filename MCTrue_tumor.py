...
def simulate(simulation_params, num_time_steps, e_degree, c_degree, assigned_actions=None):
    ...
    for i in range(num_patients):
        ...
        for t in range(0, num_time_steps - 1):
            ...
            # probabilities
            if args.random:
                radio_prob = args.size
                chemo_prob = args.size
            else:
                if cancer_volume[i, t] > 88:
                    radio_prob = 0.2
                    chemo_prob = 0.2
                elif cancer_volume[i, t] <= 88 and cancer_volume[i, t] > 44:
                    radio_prob = 0.1
                    chemo_prob = 0.1
                elif cancer_volume[i, t] <= 44 and cancer_volume[i, t] > 5:
                    radio_prob = 0.05
                    chemo_prob = 0.05
                else:
                    radio_prob = 0.01
...
parser = argparse.ArgumentParser(description='tumor case true')
...
parser.add_argument('--size', type=float, default=0.05, metavar='N',
                    help='random degree')
