...
class OWD_NT(nn.Module):
    def __init__(self, input_dim, output_dim, n_u, n_t, h_dim, z_dim, alpha, embedding_size=4, act_type='elu'):
        ...
        self.z_predictor.add_module('z_fc1', nn.Linear(input_dim - 1 + embedding_size, h_dim))
        ...
        self.a_predictor.add_module('a_fc2', nn.Linear(h_dim // 2, 1))
        self.a_predictor.add_module('a_sig2', nn.Sigmoid())
        '''y net'''
        self.y_predictor = nn.Sequential()
        self.y_predictor.add_module('y_fc1', nn.Linear(h_dim + 1, h_dim // 2))
        ...
        self.batchnorm = nn.BatchNorm1d(input_dim - 1 + embedding_size * 2)
    def z_net(self, nto):
