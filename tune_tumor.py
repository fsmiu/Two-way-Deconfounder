...
from tumormodels.TWD_Model import TWD
from tumormodels.TWD_TO_Model import TWD_TO
from tumormodels.OWD_NI_Model import OWD_NI
from tumormodels.OWD_NT_Model import OWD_NT
from tumormodels.TWD_MLP_Model import TWD_MLP
...
def objective(single_trial):
    ...
    # model
    if method=='TWD':
        model = TWD(input_dim=4, output_dim=6, n_u=u, n_t=t, h_dim=h_dim, z_dim=z_dim, alpha=alpha,
                       embedding_size=embedding_size, act_type='elu')
    elif method=='OWD_NI':
        model = OWD_NI(input_dim=4, output_dim=6, n_u=u, n_t=t, h_dim=h_dim, z_dim=z_dim, alpha=alpha,
                    embedding_size=embedding_size, act_type='elu')
    elif method == 'OWD_NT':
        model = OWD_NT(input_dim=4, output_dim=6, n_u=u, n_t=t, h_dim=h_dim, z_dim=z_dim, alpha=alpha,
                    embedding_size=embedding_size, act_type='elu')
    elif method == 'TWD_TO':
        model = TWD_TO(input_dim=4, output_dim=6, n_u=u, n_t=t, h_dim=h_dim, z_dim=z_dim, alpha=alpha,
                    embedding_size=embedding_size, act_type='elu')
    elif method == 'TWD_MLP':
        model = TWD_MLP(input_dim=4, output_dim=6, n_u=u, n_t=t, h_dim=h_dim, z_dim=z_dim, alpha=alpha,
                    embedding_size=embedding_size, act_type='elu')
    ...
    for epoch in range(num_epoch):
        ...
        for i, data_ in enumerate(train_dataloader):
            ...
            if args.method=='TWD_TO':
                ...
                loss1 = criterion(output1, ya.reshape(-1, 2))
                loss2 = mse_loss2 + var_loss2
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='tumor train')
    ...
    parser.add_argument('--n_trials', type=int, default=1, metavar='N',
                        help='n_trials')
    ...
    x1, x2, y1, y2 = data.iloc[:, 1:3], data.iloc[:, 3:7],data.iloc[:, 5:7],data.iloc[:, 9:12]
    ...
    y1 = y1.view(-1, 2)
    y = torch.cat((y1, y2), 1)
    x_train, x_test, y_train, y_test = train_test_split(x, y,stratify=x[:,4]+2*x[:,5], test_size=0.25, random_state=2022)
    # users and length
    u, t = max(data.iloc[:, 1]) + 1, max(data.iloc[:, 2]) + 1
    print(u, t)
    # 构造数据集
    trainDataset = sim_DataSet(x_train[:, 0], x_train[:, 1],x_train[:, 2:4], x_train[:, 4:6],y_train[:,0:2],y_train[:,2:5])
    testDataset = sim_DataSet(x_test[:, 0], x_test[:, 1],  x_test[:, 2:4], x_test[:, 4:6], y_test[:,0:2],y_test[:,2:5])
    ...
    if not os.path.exists(table_path + 'tumor/'):
        os.makedirs(table_path + 'tumor/')

    trials.to_csv(table_path + 'tumor/' + str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(
        args.e_degree) + '_' + str(args.c_degree) + '_tune'+method+'.csv')

    if Path(table_path + 'tumor/' + 'op_hyper_params.yml').exists():
        pass
    else:
        yaml.dump(dict(tumor=dict()),
                  open(table_path + 'tumor/' + 'op_hyper_params.yml', 'w'), default_flow_style=False)
    time.sleep(0.5)
    hyper_params_dict = yaml.safe_load(open(table_path + 'tumor/' + 'op_hyper_params.yml', 'r'))
    hyper_params_dict['tumor'][
        str(args.d_seed) + '_' + str(args.d_number) + "_" + str(args.d_t) + "_" + str(args.e_degree) + '_' + str(
            args.c_degree) + '_tune'+method] = best_params
    yaml.dump(hyper_params_dict, open(table_path + 'tumor/' + 'op_hyper_params.yml', 'w'), default_flow_style=False)
